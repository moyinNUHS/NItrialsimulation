...
simdata.nonconfounding <-  function(n, p.experiment, p.stdcare, p.alt, nonadhere.pop, cross.over, i){
  ...
  if (cross.over == TRUE) { # nonadherent participants cross over
    experiment.intervention = sample(c(0,1), size=n, replace=TRUE, prob=c(1-adhere.experiment[i], adhere.experiment[i]))
    stdcare.intervention = sample(c(0,1), size=n, replace=TRUE, prob=c(adhere.stdcare[i], 1-adhere.stdcare[i]))
  } else { # nonadherent participants take up inferior alternative treatments
    experiment.intervention = sample(c(2,1), size=n, replace=TRUE, prob=c(1-adhere.experiment[i], adhere.experiment[i]))
    stdcare.intervention = sample(c(0,2), size=n, replace=TRUE, prob=c(adhere.stdcare[i], 1-adhere.stdcare[i]))
  }
...
simdata.confounding <- function(n, p.experiment, p.stdcare, p.alt, nonadhere.pop,confounder.outcome, confounder.intervention, cross.over, i){
  ...
  if (confounder.outcome=="Increase likelihood") {
    ...
    shape1 =  shape2*p.alt/(1-p.alt) #mean of beta distribution is a/(a+b), a is shape1, b is shape2
    p.experiment.ind = sort(rbeta(n=(2*n), shape1 = shape1, shape2 = shape2), decreasing = TRUE) #individual probability with mean of p.alt, in decreasing order
