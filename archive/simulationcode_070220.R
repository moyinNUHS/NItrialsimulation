...
simdata.nonconfounding <- function(n, p.experiment, p.stdcare, p.alt, nonadhere.pop, cross.over, i){
  ...
  if (cross.over == TRUE) { # nonadherent participants take up inferior alternative treatments
    ...
    intervention[which(randomisation == 1) %in% which(is.na(intervention))] = 2
    intervention[which(randomisation == 0) %in% which(is.na(intervention))] = 2
  }
...
simdata.unknownconfounding <- function(n, p.experiment, p.stdcare, p.alt, nonadhere.pop,confounder.outcome, confounder.intervention, cross.over, i){
  ...
  randomisation = sample(rep(0:1, n)) #randomisation
  known = rbeta (n=n, shape1=2, shape2=2) #confounder beta distribution ranging 0-1 
  unknown = rbeta (n=n, shape1=2, shape2=2)
  ...
  if (confounder.outcome=="Increase likelihood") {
    ...
    shape1 = shape2*p.experiment/(1-p.experiment) 
    p.experiment.ind = sort(rbeta(n = 2*n, shape1 = shape1, shape2 = shape2)) #individual probability with mean of p.stdcare, in increasing order
    outcome1 =  rbinom(2*n, 1, prob=p.experiment.ind) #increasing confounder value will have increasing probability for outcome 
    ...
    shape1 =  shape2*p.stdcare/(1-p.stdcare) #mean of beta distribution is a/(a+b), a is shape1, b is shape2
    p.stdcare.ind = sort(rbeta(n=(2*n), shape1 = shape1, shape2 = shape2), decreasing = TRUE) #individual probability with mean of p.experiment, in decreasing order
    outcome0 =  rbinom(2*n, 1, prob = p.stdcare.ind) #increasing confounder value will have decreasing probability for outcome 
    
    shape1 =  shape2*p.experiment/(1-p.experiment) 
    p.experiment.ind = sort(rbeta(n = 2*n, shape1 = shape1, shape2 = shape2), decreasing = TRUE) #individual probability with mean of p.stdcare, in decreasing order
    outcome1 =  rbinom(2*n, 1, prob=p.experiment.ind) #increasing confounder value will have decreasing probability for outcome 
  ...
  d = matrix(data=c(id, randomisation, confounder, known, unknown), ncol = 5)
  ...
  if (confounder.intervention=="Increase likelihood") {
    ...
    shape1 =  shape2*adhere.stdcare[i]/(1 - adhere.stdcare[i]) 
    adhere.stdcare.ind = sort(rbeta(n = n, shape1 = shape1, shape2 = shape2), decreasing = TRUE) #individual probability with mean of p.stdcare, in decreasing order
    cont.intervention =  rbinom(n, 1, prob = adhere.stdcare.ind)
  ...
  simdata = matrix(cbind(d.grouped[,-6:-8], intervention, outcome), ncol = 7)
