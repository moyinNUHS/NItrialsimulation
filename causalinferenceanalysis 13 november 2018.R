...
library(tableone); library(MatchIt); library(geepack); library(scales)

# The colour blind friendly palette with black:
cbPalette <- c("#E69F00", "#56B4E9", "#009E73", "#0072B2", "#D55E00", "#CC79A7","#999999")
col.itt<- "#009E73" ; col.pp <- "#D55E00"; col.at<-  "#E69F00" ; col.iv<-  "#56B4E9"; col.mpp<- "#0072B2" ; col.ps<-  "#CC79A7"; col.matched.iv<-"#999999"
...
#################CASE 1.1 Non-compliance caused by non confounding process (affect both groups) #########################
################################################################################################################

#BIAS
simdata.bias.1.1<- function(n, i0, i1, p1, p0, confounder.eff.i,confounder.eff.o, confounder.u.eff.i,confounder.u.eff.o, nIterations, interval){  
    
    #make up vectors for simulations 
    .eff.iv<-.eff.itt<-.eff.pp<-.eff.at<-.eff.mpp<-.eff.ps<-c() #output from each simulation
    eff.iv<-eff.itt<-eff.pp<-eff.at<-eff.mpp<-eff.ps<-c()       #for saving output from each simulation
    prop.c<-.prop.c<-c()                                        #proportion of compliant particpants from each simulation 
        ...
        for(i in 1:interval) { print(paste ("interval value",i))
            for(l in 1:nIterations) { 
                tryCatch({ #allow the function to run in case of errors
                    ...
                    if (true.eff>=0) { # conterfactual outcomes differ by true effect 
                    ...
                    #INTERVENTION dependent on randomisation and a non confoundingprocess - creating increasing compliance
                    ...
                    mout<-matchit(intervention~confounder, data=simdata, method='nearest', distance = "mahalanobis", replace = TRUE)
    ...
    ps<-ggplot(ps.melt, aes(x=prop.c, y=value))+ 
        geom_point(aes(y=ps.melt$value, colour="Mahalanobis matching", alpha=0.01), size=0.2) + 
        geom_smooth(aes(y=ps.melt$value, colour="Mahalanobis matching"), method = lm, formula = y ~ splines::bs(x, 3), se = FALSE)+
        xlab("Proportion of compliant participants")+
        ylab("Effect")+
        theme_bw()+
...
#TYPE 1 ERROR
simdata.type1.1.1<- function(n, i0, i1, NImargin, p1, confounder.eff.i,confounder.eff.o, confounder.u.eff.i,confounder.u.eff.o, nIterations,interval){  
    ...
    if (NImargin>0) {
        ...
        plot <- ggplot(sim.df, aes(sim.df[1]))+ 
            ...
            geom_point(aes(y=sim.df[7], colour="Mahalanobis matching", alpha=0.01)) + 
            ...
            geom_smooth(aes(y=sim.df[7], colour="Mahalanobis matching"), method = lm, formula = y ~ splines::bs(x, 3), se = FALSE)+
            xlab("Proportion of compliant participants")+
            ylab("Type 1 error")+
            theme_bw()+
...
#POWER
simdata.power.1.1<- function(n, i0, i1, p1, p0, confounder.eff.o, confounder.eff.i, confounder.u.eff.i,confounder.u.eff.o, NImargin, nIterations,interval){  
    
    #make up vectors for simulations 
    .power.at<-.power.itt<-.power.iv<-.power.mpp<-.power.pp<-.power.ps<-c() 
    power.at<-power.itt<-power.iv<-power.mpp<-power.pp<-power.ps<-c() 
    prop.c<-.prop.c<-c()
    ...
    if ((NImargin>0) & (true.eff < NImargin)) { #built with alternative hypothesis: true effect < NI 
        ...
        sim<- function() { 
            for(i in 1:interval) { print(paste ("interval value",i))
                ...
                power.mpp[i]<- mean(.power.mpp, na.rm=TRUE)
                power.iv[i] <- mean(.power.iv, na.rm=TRUE)
                power.ps[i] <- mean(.power.ps, na.rm=TRUE)
...
b1.1<-simdata.bias.1.1   (n=300, p1=0.3, p0=0.4, i0=0.01, i1=0.99, confounder.eff.i= 1, confounder.eff.o = 1, confounder.u.eff.i=1, confounder.u.eff.o=1, nIterations=200, interval = interval)
t1.1<-simdata.type1.1.1  (n=300, p1=0.3, NImargin=0.05, i0=0.01, i1=0.99, confounder.eff.i= 1, confounder.eff.o = 1, confounder.u.eff.i=1, confounder.u.eff.o=1, nIterations=200, interval = interval)
p1.1<- simdata.power.1.1 (n=300, p1=0.3, p0=0.4, NImargin=0.05, i0=0.01, i1=0.99, confounder.eff.i= 1, confounder.eff.o = 1,  confounder.u.eff.i=1,confounder.u.eff.o=1,nIterations=200, interval = interval)
case1.1<-ggarrange(b1.1, 
                   ggarrange (t1.1, p1.1, ncol = 2, labels = c(paste(LETTERS[2:3], sep="")), legend=NULL), 
                   nrow=2, 
                   labels ='A',
                   common.legend = TRUE, legend = "bottom" )
annotate_figure(case1.1, top = text_grob("Case 1.1: Non-compliance caused by non confounding processes affecting both groups", face = "bold", size = 12))
ggsave(filename = paste("case1.1", Sys.Date(), ".pdf"), width = 13.9, height = 10.2, units = c("in"))

#################CASE 1.2 Non-compliance caused by non confounding process (affect intervention or control group) ##################
################################################################################################################
#BIAS
simdata.bias.1.2<- function(n, i0, i1, p0, p1, confounder.eff.i,confounder.eff.o, confounder.u.eff.i,confounder.u.eff.o, nIterations, interval){  
        ...
        for(i in 1:interval) { print(paste ("interval value",i))
            for(l in 1:nIterations) { 
                tryCatch({ #allow the function to run in case of errors
                    ...
                    #INTERVENTION dependent on randomisation and a non confounding process - creating increasing compliance
                    simdata$intervention <- rep(NA,2*n)
...
#TYPE 1 ERROR
simdata.type1.1.2<- function(n, i0, i1, NImargin, p1, confounder.eff.i,confounder.eff.o, confounder.u.eff.i,confounder.u.eff.o, nIterations, interval){  
    # NImargin: non inferiority margin 
...
#POWER
simdata.power.1.2<- function(n, i0, i1, p1, p0, confounder.eff.o, confounder.eff.i, confounder.u.eff.i,confounder.u.eff.o, NImargin, nIterations, interval){  
...
b1.2<-simdata.bias.1.2   (n=300, p1=0.3, p0=0.4,       i0=0.01, i1=0.99, confounder.eff.i= 1, confounder.eff.o = 1, confounder.u.eff.i=1, confounder.u.eff.o=1, nIterations=200, interval = interval)
t1.2<-simdata.type1.1.2  (n=300, p1=0.3, NImargin=0.05, i0=0.01, i1=0.99, confounder.eff.i= 1, confounder.eff.o = 1, confounder.u.eff.i=1, confounder.u.eff.o=1, nIterations=200, interval = interval)
p1.2<- simdata.power.1.2 (n=300, p1=0.3, p0=0.4, NImargin=0.05, i0=0.01, i1=0.99, confounder.eff.i= 1, confounder.eff.o = 1,  confounder.u.eff.i=1,confounder.u.eff.o=1,nIterations=200, interval = interval)
...
simdata.bias.2<- function(n, i0, i1, p0, p1, confounder.eff.i,confounder.eff.o, confounder.u.eff.i,confounder.u.eff.o, nIterations, interval){  
    ...
    #make up vectors for simulations 
    eff.iv<-eff.itt<-eff.pp<-eff.at<-eff.mpp<-eff.ps<-eff.matched.iv<-c()                   
    .eff.iv<-.eff.itt<-.eff.pp<-.eff.at<-.eff.mpp<-.eff.ps<-.eff.matched.iv<-c() 
    prop.c<-.prop.c<-c()
    ...
    sim<- function() { 
        for(i in 1:interval) { print(paste ("interval value",i))
            for(l in 1:nIterations) { 
                tryCatch({ #allow the function to run in case of errors
                    ...
                    #m<- glm(intervention~confounder, family = binomial(), data=simdata) 
                    #pscore<- m$fitted.values
                    mout<-matchit(intervention~confounder,data=simdata,	mahvars	= "confounder", method = 'full',caliper= 0.2, distance= "mahalanobis", replace=TRUE)
                    #match<-Match(Tr=simdata$intervention, M=1, X= logit(pscore), replace=FALSE, caliper=0.2)
                    ...
                    #matched IV 
                    matched.y<- matched$outcome
                    matched.x<- matched$intervention
                    matched.iv<-matched$randomisation
                    matched.data<-data.frame(matched.y,matched.x,matched.iv)
                    matched.asmm <- gmm(matched.data[,"matched.y"] ~ matched.data[,"matched.x"], x=matched.data[,"matched.iv"], vcov="iid")
                    .eff.matched.iv[l]<-(summary(matched.asmm))$ coefficients [2,1]
            ...
            eff.mpp[[i]]<- .eff.mpp
            eff.ps[[i]]<- .eff.ps
            eff.matched.iv[[i]]<- .eff.matched.iv
        ...
        eff.mpp.df<- cbind(prop.c,t(data.frame(eff.mpp))) 
        eff.ps.df<- cbind(prop.c,t(data.frame(eff.ps))) 
        eff.matched.iv.df<- cbind(prop.c,t(data.frame(eff.matched.iv))) 
        
        return(list(eff.itt.df, eff.pp.df,eff.at.df,eff.iv.df,eff.mpp.df,eff.ps.df, eff.matched.iv.df))
    }
    ...
    matched.iv.df<-as.data.frame(sim.df[7])
    colnames(matched.iv.df)<- c('prop.c', 1:nIterations)
    row.names(matched.iv.df)<- c(1:interval)
    matched.iv.melt<- melt(matched.iv.df, id.vars='prop.c')
    matched.iv<-ggplot(matched.iv.melt, aes(x=prop.c, y=value))+ 
        geom_point(aes(y=matched.iv.melt$value, colour="Matched IV", alpha=0.01), size=0.2) + 
        geom_smooth(aes(y=matched.iv.melt$value, colour="Matched IV"), method = lm, formula = y ~ splines::bs(x, 3), se = FALSE)+
        xlab("Proportion of compliant participants")+
        ylab("Effect")+
        theme_bw()+
        scale_colour_manual(values=col.matched.iv)+
        theme(legend.title=element_blank(), legend.position="none")+
        scale_x_continuous(limits=c(0.60, 1))+
        scale_y_continuous(limits=c(-0.4, 0.2))
    
    bias.plot<-ggarrange(itt, pp, at,
                         iv, mpp, ps,matched.iv,
                         ncol = 3, nrow = 3,
                         common.legend = FALSE )
...
#TYPE 1 ERROR
simdata.type1.2<- function(n, i0, i1, NImargin, p1, confounder.eff.i,confounder.eff.o, confounder.u.eff.i,confounder.u.eff.o, nIterations, interval){  
    # NImargin: non inferiority margin 
    ...
    if (NImargin>0) {
        ...
        plot <- ggplot(sim.df, aes(sim.df[1]))+ 
            ...
            theme(legend.title=element_blank())+
            scale_x_continuous(limits=c(0.6, 1))+
            scale_y_continuous(limits=c(0,0.75), breaks=seq(0,75,0.1))
...
#POWER
simdata.power.2<- function(n, i0, i1, p1, p0, confounder.eff.o, confounder.eff.i, confounder.u.eff.i,confounder.u.eff.o, NImargin, nIterations, interval){  
...
b2.11<-simdata.bias.2   (n=300, p1=0.3, p0=0.4, i0=0.45, i1=0.55, confounder.eff.i= 0.001,     confounder.eff.o = 1000,  confounder.u.eff.i=1,confounder.u.eff.o=1,nIterations=200, interval = interval)
t2.11<-simdata.type1.2  (n=300, p1=0.3, NImargin=0.05, i0=0.45, i1=0.55, confounder.eff.i= 0.001, confounder.eff.o = 1000, confounder.u.eff.i=1, confounder.u.eff.o=1, nIterations=200, interval = interval)
p2.11<- simdata.power.2 (n=300, p1=0.3, p0=0.4, NImargin=0.05, i0=0.45, i1=0.55, confounder.eff.i= 0.001, confounder.eff.o = 1000,  confounder.u.eff.i=1,confounder.u.eff.o=1,nIterations=200, interval = interval)
...
b2.12<-simdata.bias.2   (n=300, p1=0.3, p0=0.4, i0=0.45, i1=0.55, confounder.eff.i= 1000,     confounder.eff.o = 1000,  confounder.u.eff.i=1,confounder.u.eff.o=1,nIterations=200, interval = interval)
t2.12<-simdata.type1.2  (n=300, p1=0.3, NImargin=0.05, i0=0.45, i1=0.55, confounder.eff.i= 1000, confounder.eff.o = 1000, confounder.u.eff.i=1, confounder.u.eff.o=1, nIterations=200, interval = interval)
p2.12<- simdata.power.2 (n=300, p1=0.3, p0=0.4, NImargin=0.05, i0=0.45, i1=0.55, confounder.eff.i= 1000, confounder.eff.o = 1000,  confounder.u.eff.i=1,confounder.u.eff.o=1,nIterations=200, interval = interval)
...
b2.13<-simdata.bias.2   (n=300, p1=0.3, p0=0.4, i0=0.45, i1=0.55, confounder.eff.i= 0.001,     confounder.eff.o = 0.1,  confounder.u.eff.i=1,confounder.u.eff.o=1,nIterations=200, interval = interval)
t2.13<-simdata.type1.2  (n=300, p1=0.3, NImargin=0.05, i0=0.45, i1=0.55, confounder.eff.i= 0.001, confounder.eff.o = 0.1, confounder.u.eff.i=1, confounder.u.eff.o=1, nIterations=200, interval = interval)
p2.13<- simdata.power.2 (n=300, p1=0.3, p0=0.4, NImargin=0.05, i0=0.45, i1=0.55, confounder.eff.i= 0.001, confounder.eff.o = 0.1,  confounder.u.eff.i=1,confounder.u.eff.o=1,nIterations=200, interval = interval)
...
b2.14<-simdata.bias.2   (n=300, p1=0.3, p0=0.4, i0=0.45, i1=0.55, confounder.eff.i= 1000,     confounder.eff.o = 0.1,  confounder.u.eff.i=1,confounder.u.eff.o=1,nIterations=200, interval = interval)
t2.14<-simdata.type1.2  (n=300, p1=0.3, NImargin=0.05, i0=0.45, i1=0.55, confounder.eff.i= 1000, confounder.eff.o = 0.1, confounder.u.eff.i=1, confounder.u.eff.o=1, nIterations=200, interval = interval)
p2.14<- simdata.power.2 (n=300, p1=0.3, p0=0.4, NImargin=0.05, i0=0.45, i1=0.55, confounder.eff.i= 1000, confounder.eff.o = 0.1,  confounder.u.eff.i=1,confounder.u.eff.o=1,nIterations=200, interval = interval)
...
b2.21<-simdata.bias.2   (n=300, p1=0.3, p0=0.4,       i0=0.01, i1=0.99, confounder.eff.i= 0.000001, confounder.eff.o = 1000,  confounder.u.eff.i=1,confounder.u.eff.o=1,nIterations=200, interval = interval)
t2.21<-simdata.type1.2  (n=300, p1=0.3, NImargin=0.05, i0=0.01, i1=0.99, confounder.eff.i= 0.000001, confounder.eff.o = 1000, confounder.u.eff.i=1, confounder.u.eff.o=1, nIterations=200, interval = interval)
p2.21<- simdata.power.2 (n=300, p1=0.3, p0=0.4, NImargin=0.05, i0=0.01, i1=0.99, confounder.eff.i= 0.000001, confounder.eff.o = 100,  confounder.u.eff.i=1,confounder.u.eff.o=1,nIterations=200, interval = interval)
...
b2.22<-simdata.bias.2   (n=300, p1=0.3, p0=0.4,       i0=0.01, i1=0.99, confounder.eff.i= 0.000001, confounder.eff.o = 0.1,  confounder.u.eff.i=1,confounder.u.eff.o=1,nIterations=200, interval = interval)
t2.22<-simdata.type1.2  (n=300, p1=0.3, NImargin=0.05, i0=0.01, i1=0.99, confounder.eff.i= 0.000001, confounder.eff.o = 0.1, confounder.u.eff.i=1, confounder.u.eff.o=1, nIterations=200, interval = interval)
p2.22<- simdata.power.2 (n=300, p1=0.3, p0=0.4, NImargin=0.05, i0=0.01, i1=0.99, confounder.eff.i= 0.000001, confounder.eff.o = 0.1,  confounder.u.eff.i=1,confounder.u.eff.o=1,nIterations=200, interval = interval)
case2.22<-ggarrange(b2.22, 
                    ggarrange (t2.22, p2.22, ncol = 2, labels = c(paste(LETTERS[2:3], sep="")), legend=NULL), 
                    nrow=2, 
                    labels ='A',
                    common.legend = TRUE, legend = "bottom" )
annotate_figure(case2.22, top = text_grob("Case 2.22: Non-compliance caused by confounding process (affect intervention group)-higher value of confounder makes outcome less likely", face = "bold", size = 12))
ggsave(filename = paste("case2.22", Sys.Date(), ".pdf"), width = 13.9, height = 10.2, units = c("in"))

#Run simulations for case 2.3: Non-compliance caused by confounding process (affect control groups)
b2.31<-simdata.bias.2   (n=300, p1=0.3, p0=0.4,       i0=0.01, i1=0.99, confounder.eff.i= 1000000, confounder.eff.o = 100,  confounder.u.eff.i=1,confounder.u.eff.o=1,nIterations=200, interval = interval)
t2.31<-simdata.type1.2  (n=300, p1=0.3, NImargin=0.05, i0=0.01, i1=0.99, confounder.eff.i= 1000000, confounder.eff.o = 100, confounder.u.eff.i=1, confounder.u.eff.o=1, nIterations=200, interval = interval)
p2.31<- simdata.power.2 (n=300, p1=0.3, p0=0.4, NImargin=0.05, i0=0.01, i1=0.99, confounder.eff.i= 1000000, confounder.eff.o = 100,  confounder.u.eff.i=1,confounder.u.eff.o=1,nIterations=200, interval = interval)
case2.31<-ggarrange(b2.31, 
                    ggarrange (t2.31, p2.31, ncol = 2, labels = c(paste(LETTERS[2:3], sep="")), legend=NULL), 
                    nrow=2, 
                    labels ='A',
                    common.legend = TRUE, legend = "bottom" )
annotate_figure(case2.31, top = text_grob("Case 2.31: Non-compliance caused by confounding process (affect control group)-higher value of confounder makes outcome more likely", face = "bold", size = 12))
ggsave(filename = paste("case2.31", Sys.Date(), ".pdf"), width = 13.9, height = 10.2, units = c("in"))

b2.32<-simdata.bias.2   (n=300, p1=0.3, p0=0.4,       i0=0.01, i1=0.99, confounder.eff.i= 1000000, confounder.eff.o = 0.1,  confounder.u.eff.i=1,confounder.u.eff.o=1,nIterations=200, interval = interval)
t2.32<-simdata.type1.2  (n=300, p1=0.3, NImargin=0.05, i0=0.01, i1=0.99, confounder.eff.i= 1000000, confounder.eff.o = 0.1, confounder.u.eff.i=1, confounder.u.eff.o=1, nIterations=200, interval = interval)
p2.32<- simdata.power.2 (n=300, p1=0.3, p0=0.4, NImargin=0.05, i0=0.01, i1=0.99, confounder.eff.i= 1000000, confounder.eff.o = 0.1,  confounder.u.eff.i=1,confounder.u.eff.o=1,nIterations=200, interval = interval)
case2.32<-ggarrange(b2.32, 
                    ggarrange (t2.32, p2.32, ncol = 2, labels = c(paste(LETTERS[2:3], sep="")), legend=NULL), 
                    nrow=2, 
                    labels ='A',
                    common.legend = TRUE, legend = "bottom" )
annotate_figure(case2.32, top = text_grob("Case 2.32: Non-compliance caused by confounding process (affect control group)-higher value of confounder makes outcome less likely", face = "bold", size = 12))
ggsave(filename = paste("case2.32", Sys.Date(), ".pdf"), width = 13.9, height = 10.2, units = c("in"))

#Run simulations for case 2.4: Non-compliance caused by confounding process (affect control groups) with undocumented confounders
b2.4<-simdata.bias.2   (n=300, p1=0.3, p0=0.4,       i0=0.01, i1=0.99, confounder.eff.i= 10, confounder.eff.o = 2,  confounder.u.eff.i=100000,confounder.u.eff.o=98,nIterations=50, interval = interval)
t2.4<-simdata.type1.2  (n=300, p1=0.3, NImargin=0.05, i0=0.01, i1=0.99, confounder.eff.i= 10, confounder.eff.o = 2, confounder.u.eff.i=100000, confounder.u.eff.o=98, nIterations=200, interval = interval)
p2.4<- simdata.power.2 (n=300, p1=0.3, p0=0.4, NImargin=0.05, i0=0.01, i1=0.99, confounder.eff.i= 10, confounder.eff.o = 2,  confounder.u.eff.i=100000,confounder.u.eff.o=98,nIterations=200, interval = interval)
case2.4<-ggarrange(b2.4, 
                   ggarrange (t2.4, p2.4, ncol = 2, labels = c(paste(LETTERS[2:3], sep="")), legend=NULL), 
                   nrow=2, 
                   labels ='A',
                   common.legend = TRUE, legend = "bottom" )
annotate_figure(case2.4, top = text_grob("Case 2.4: Non-compliance caused by confounding process (affect control group) with undocumented confounder", face = "bold", size = 12))
ggsave(filename = paste("case2.4", Sys.Date(), ".pdf"), width = 13.9, height = 10.2, units = c("in"))

##Run simulations for case 2.51: Impact of Ni margin on bias and power 
# Based on case 2.14: Non-compliance caused by confounding process (affect both groups)
b2.51<-simdata.bias.2   (n=300, p1=0.3, p0=0.4, i0=0.45, i1=0.55, confounder.eff.i= 1000,     confounder.eff.o = 0.1,  confounder.u.eff.i=1,confounder.u.eff.o=1,nIterations=200, interval = interval)
t2.51<-simdata.type1.2  (n=300, p1=0.3, NImargin=0.1, i0=0.45, i1=0.55, confounder.eff.i= 1000, confounder.eff.o = 0.1, confounder.u.eff.i=1, confounder.u.eff.o=1, nIterations=200, interval = interval)
p2.51<- simdata.power.2 (n=300, p1=0.3, p0=0.4, NImargin=0.1, i0=0.45, i1=0.55, confounder.eff.i= 1000, confounder.eff.o = 0.1,  confounder.u.eff.i=1,confounder.u.eff.o=1,nIterations=200, interval = interval)
case2.51<-ggarrange(b2.51, 
                    ggarrange (t2.51, p2.51, ncol = 2, labels = c(paste(LETTERS[2:3], sep="")), legend=NULL), 
                    nrow=2, 
                    labels ='A',
                    common.legend = TRUE, legend = "bottom" )
annotate_figure(case2.51, top = text_grob("Case 2.51: Non-compliance caused by confounding process (affect both groups)-higher value of confounder makes intervention more likely, outcome less likely, NI margin=0.1", face = "bold", size = 12))
ggsave(filename = paste("case2.51", Sys.Date(), ".pdf"), width = 13.9, height = 10.2, units = c("in"))

##Run simulations for case 2.52: Impact of sample size on bias and power 
# Based on case 2.14: Non-compliance caused by confounding process (affect both groups)
b2.52<-simdata.bias.2   (n=3000, p1=0.3, p0=0.4, i0=0.45, i1=0.55, confounder.eff.i= 1000,     confounder.eff.o = 0.1,  confounder.u.eff.i=1,confounder.u.eff.o=1,nIterations=200, interval = interval)
t2.52<-simdata.type1.2  (n=3000, p1=0.3, NImargin=0.05, i0=0.45, i1=0.55, confounder.eff.i= 1000, confounder.eff.o = 0.1, confounder.u.eff.i=1, confounder.u.eff.o=1, nIterations=200, interval = interval)
p2.52<- simdata.power.2 (n=3000, p1=0.3, p0=0.4, NImargin=0.05, i0=0.45, i1=0.55, confounder.eff.i= 1000, confounder.eff.o = 0.1,  confounder.u.eff.i=1,confounder.u.eff.o=1,nIterations=200, interval = interval)
case2.52<-ggarrange(b2.52, 
                    ggarrange (t2.52, p2.52, ncol = 2, labels = c(paste(LETTERS[2:3], sep="")), legend=NULL), 
                    nrow=2, 
                    labels ='A',
                    common.legend = TRUE, legend = "bottom" )
annotate_figure(case2.52, top = text_grob("Case 2.52: Non-compliance caused by confounding process (affect both groups)-higher value of confounder makes intervention more likely, outcome less likely, sample size=3000", face = "bold", size = 12))
ggsave(filename = paste("case2.52", Sys.Date(), ".pdf"), width = 13.9, height = 10.2, units = c("in"))

##Run simulations for case 2.53: Impact of effect size on bias and power 
# Based on case 2.14: Non-compliance caused by confounding process (affect both groups)
b2.53<-simdata.bias.2   (n=300, p1=0.35, p0=0.4, i0=0.45, i1=0.55, confounder.eff.i= 1000,     confounder.eff.o = 0.1,  confounder.u.eff.i=1,confounder.u.eff.o=1,nIterations=200, interval = interval)
t2.53<-simdata.type1.2  (n=300, p1=0.35, NImargin=0.5, i0=0.45, i1=0.55, confounder.eff.i= 1000, confounder.eff.o = 0.1, confounder.u.eff.i=1, confounder.u.eff.o=1, nIterations=200, interval = interval)
p2.53<- simdata.power.2 (n=300, p1=0.35, p0=0.4, NImargin=0.5, i0=0.45, i1=0.55, confounder.eff.i= 1000, confounder.eff.o = 0.1,  confounder.u.eff.i=1,confounder.u.eff.o=1,nIterations=200, interval = interval)
case2.53<-ggarrange(b2.53, 
                    ggarrange (t2.53, p2.53, ncol = 2, labels = c(paste(LETTERS[2:3], sep="")), legend=NULL), 
                    nrow=2, 
                    labels ='A',
                    common.legend = TRUE, legend = "bottom" )
annotate_figure(case2.53, top = text_grob("Case 2.53: Non-compliance caused by confounding process (affect both groups)-higher value of confounder makes intervention more likely, outcome less likely, effect size =-0.15", face = "bold", size = 12))
ggsave(filename = paste("case2.53", Sys.Date(), ".pdf"), width = 13.9, height = 10.2, units = c("in"))
